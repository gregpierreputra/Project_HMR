...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        # 1st Layer
        self.discriminator_pose_conv1 = nn.Conv2d(
            in_channels=9, out_channels=32, kernel_size=1
        )
        nn.init.xavier_uniform_(self.discriminator_pose_conv1.weight)
        nn.init.zeros_(self.discriminator_pose_conv1.bias)

        # 2nd Layer
        self.discriminator_pose_conv2 = nn.Conv2d(
        ...
        for _ in range(self.number_of_joints):
            pose_out_temperature = nn.Linear(in_features=32, out_features=1)
        ...
        # 1st Layer
        self.discriminator_body_fc1 = nn.Linear(
            in_features=32 * self.number_of_joints, out_features=1024
        )
        nn.init.xavier_uniform_(self.discriminator_body_fc1.weight)
        nn.init.zeros_(self.discriminator_body_fc1.bias)

        # 2nd Layer
        self.discriminator_body_fc2 = nn.Linear(in_features=1024, out_features=1024)
        nn.init.xavier_uniform_(self.discriminator_body_fc2.weight)
        nn.init.zeros_(self.discriminator_body_fc2.bias)

        # Output Layer
        self.discriminator_body_out = nn.Linear(in_features=1024, out_features=1)
    ...
    def forward(self, poses: torch.Tensor, betas: torch.Tensor) -> torch.Tensor:
        ...
        discriminators_out = torch.cat((poses_out, betas_out, poses_all_out), dim=1)
