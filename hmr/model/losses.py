...
class Keypoint2DLoss(nn.Module):
    ...
    def forward(self,
        ...
        loss = (conf * self.loss_function(
            projected_keypoints_2d,
            ground_truth_keypoints_2d[:, :, :-1]
        )).sum(dim=(1, 2))
...
class Keypoint3DLoss(nn.Module):
    ...
    def forward(self,
        ...
        projected_keypoints_3d = projected_keypoints_3d - projected_keypoints_3d[:, pelvis_id, :].unsqueeze(1)

        # For all 3D ground truth keypoints except the last one, remove 3D pelvis keypoint
        # and add additional dimension at the end to keep size as [B, 1, S, N]
        # Keep all keypoints except the last one
        ground_truth_keypoints_3d[:, :, :-1] = ground_truth_keypoints_3d[:, :, :-1] - ground_truth_keypoints_3d[:, pelvis_id, :-1].unsqueeze(1)

        conf = ground_truth_keypoints_3d[:, :, -1].unsqueeze(-1).clone()
        
        loss = (conf * self.loss_function(
            projected_keypoints_3d,
            ground_truth_keypoints_3d[:, :, :-1]
        )).sum(dim=(1,2))
...
class ParameterLoss(nn.Module):
    ...
    def forward(self,
        ...
        has_parameter = has_parameter.type(projected_parameter.type()).view(*mask_dimension)
