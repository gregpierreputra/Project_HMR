...
class SMPLTransformerDecoderHead(nn.Module):
    ...
    def __init__(
        ...
        smpl_joint_rep: str = "6d",
        smpl_num_body_joints: int = 23,
        smpl_transformer_input: str = "zero",
        init_decoder_xavier: bool = False,
        smpl_ief_iters: int = 1,
    ):
        super().__init__()
        self.joint_rep_type = smpl_joint_rep
        self.joint_rep_dim = {"6d": 6, "aa": 3}[self.joint_rep_type]
        self.smpl_num_body_joints = smpl_num_body_joints

        npose = self.joint_rep_dim * (self.smpl_num_body_joints + 1)
        self.npose = npose
        self.smpl_ief_iters = smpl_ief_iters

        self.input_is_mean_shape = smpl_transformer_input == "mean_shape"
    ...
    def forward(self, x, **kwargs):
        ...
        pred_body_pose = joint_conversion_fn(pred_body_pose).view(
            batch_size, self.smpl_num_body_joints + 1, 3, 3
        )
